...
async def register_user(payload: RegisterIn):
    # TODO: check DB for existing, hash password, create user, create Stripe customer, etc.
    return {"email": payload.email, "created": True}
from fastapi.middleware.cors import CORSMiddleware

app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://useleadnest.com", "http://localhost:3000"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
from pydantic import BaseModel, EmailStr  # or just str if you prefer
...
@app.post("/api/auth/register")
async def register_user(payload: RegisterIn):
    # TODO: real logic (check existing, hash, save, etc.)
    return {"email": payload.email, "created": True}
pydantic[email]
# or
email-validator
